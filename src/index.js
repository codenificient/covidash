  ...
  const makeRequest = async (method, ...args) => {
    ...
    try {
      ...
      if (retryCount < maxRetries) {
        console.warn(`Analytics ${method} failed, retrying... (${retryCount}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, 1000 * retryCount)); // Exponential backoff
        return makeRequest(method, ...args);
      }
      
      console.warn(`Analytics ${method} failed after ${maxRetries} retries:`, error);
      return Promise.resolve();
    }
  };
...
const localAnalytics = {
  store: (event, data) => {
    try {
      ...
      localStorage.setItem('local_analytics', JSON.stringify(events.slice(-100))); // Keep last 100 events
    } catch (error) {
